<?php
  ...
  $box_similar_products_cache_token = cache::token('box_similar_products', [$_GET['product_id'], !empty($_GET['category_id']) ? $_GET['category_id'] : implode(', ', array_keys($product->categories)), 'language', 'currency'], 'file');
  if (cache::capture($box_similar_products_cache_token)) {
    ...
    if (database::num_rows($products_query) > 0) {
      ...
      $box_similar_products = new ent_view('views/box_similar_products.inc.php');
