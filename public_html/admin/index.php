...
// Build apps list menu
  ...
  foreach (functions::admin_get_apps() as $app) {
    ...
    $apps_list .= '    <li'. ((isset($_GET['app']) && $_GET['app'] == $app['code']) ? ' class="selected"' : '') .'>'. PHP_EOL .'      <a href="'. document::href_link(WS_DIR_ADMIN, $params) .'"><img src="'. WS_DIR_ADMIN . $app['code'] .'.app/'. $app['icon'] .'" width="24" height="24" style="vertical-align: middle;" alt="'. $app['name'] .'" title="'. $app['name'] .'" /> '. $app['name'] .'</a>' . PHP_EOL;
    ...
    if (!empty($_GET['app']) && $_GET['app'] == $app['code']) {
      ...
      if (!empty($app['menu'])) {
        ...
        foreach ($app['menu'] as $item) {
          ...
          $apps_list .= '        <li'. ($selected ? ' class="selected"' : '') .'><a href="'. document::href_link(WS_DIR_ADMIN, $params) .'">'. $item['title'] .'</a></li>' . PHP_EOL;
        }
