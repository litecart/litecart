<?php
  ...
  if (!empty($_POST['enable']) || !empty($_POST['disable'])) {
    ...
    if (!empty($_POST['languages'])) {
      foreach ($_POST['languages'] as $key => $value) $_POST['languages'][$key] = database::input($value);
      database::query(
...
?>
  <tr class="<?php echo $rowclass . ($language['status'] ? false : ' semi-transparent'); ?>">
    <td nowrap="nowrap"><img src="<?php echo WS_DIR_IMAGES .'icons/16x16/'. (!empty($language['status']) ? 'on.png' : 'off.png') ?>" width="16" height="16" align="absbottom" /> <?php echo functions::form_draw_checkbox('languages['. $language['id'] .']', $language['id']); ?></td>
