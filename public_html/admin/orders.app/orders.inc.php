...
?>
    ...
    <td><a href="<?php echo document::href_link('', array('doc' => 'edit_order', 'order_id' => $order['id']), true); ?>"><?php echo $order['customer_company'] ? $order['customer_company'] : $order['customer_firstname'] .' '. $order['customer_lastname']; ?><?php echo empty($order['customer_id']) ? ' <em>('. language::translate('title_guest', 'Guest') .')</em>' : ''; ?></a> <span style="opacity: 0.5;"><?php echo $order['customer_tax_id']; ?></span></td>
      ...
      <td>
        <a data-toggle="lightbox" data-type="iframe" data-width="600px" data-height="800px" href="<?php echo document::href_link('', array('doc' => 'printable_packing_slip', 'order_id' => $order['id'], 'media' => 'print'), array('app')); ?>"><?php echo functions::draw_fonticon('fa-file-text-o'); ?></a>
        <a data-toggle="lightbox" data-type="iframe" data-width="600px" data-height="800px" href="<?php echo document::href_link('', array('doc' => 'printable_order_copy', 'order_id' => $order['id'], 'media' => 'print'), array('app')); ?>"><?php echo functions::draw_fonticon('fa-print'); ?></a>
