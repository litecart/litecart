...
?>
<div style="float: right;"><a class="button" href="<?php echo $system->document->link('', array('doc' => 'edit_tax_rate.php'), true); ?>"><?php echo $system->language->translate('title_add_new_tax_rate', 'Add New Tax Rate'); ?></a></div>
...
<?php
  ...
  // Jump to data for current page
    if ($_GET['page'] > 1) $system->database->seek($tax_rates_query, ($system->settings->get('data_table_rows_per_page', 20) * ($_GET['page']-1)));
...
?>
  <tr class="<?php echo $rowclass; ?>">
    ...
    <td align="right"><a href="<?php echo $system->document->link('', array('doc' => 'edit_tax_rate.php', 'tax_rate_id' => $tax_rate['id']), true); ?>"><img src="<?php echo WS_DIR_IMAGES . 'icons/16x16/edit.png'; ?>" width="16" height="16" border="0" title="<?php echo $system->language->translate('title_edit', 'Edit'); ?>" /></a></td>
  </tr>
<?php
...
// Display page links
  echo $system->functions->draw_pagination(ceil($system->database->num_rows($tax_rates_query)/$system->settings->get('data_table_rows_per_page', 20)));
?>