...
?>
  <tr class="<?php echo $rowclass . (($product['status']) ? false : ' semi-transparent'); ?>">
    ...
    <td><?php echo '<img src="'. (!empty($product['image']) ? $system->functions->image_resample(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . $product['image'], FS_DIR_HTTP_ROOT . WS_DIR_CACHE, 16, 16, 'FIT_USE_WHITESPACING') : WS_DIR_IMAGES .'no_image.png') .'" width="16" height="16" align="absbottom" />'; ?><a href="<?php echo $system->document->href_link('', array('app' => $_GET['app'], 'doc' => 'edit_product', 'product_id' => $product['id'])); ?>"> <?php echo $product['name']; ?></a></td>
    <td align="right" nowrap="nowrap"></td>
    <td><a href="<?php echo $system->document->href_link('', array('app' => $_GET['app'], 'doc' => 'edit_product', 'product_id' => $product['id'])); ?>"><img src="<?php echo WS_DIR_IMAGES; ?>icons/16x16/edit.png" width="16" height="16" alt="<?php echo $system->language->translate('title_edit', 'Edit'); ?>" align="absbottom" /></a></td>
  </tr>
<?php
      }
    }
...
<?php
    ...
    function admin_catalog_category_products($category_id=0, $depth=1) {
      ...
      $products_query = $GLOBALS['system']->database->query(
        ...
        left join ". DB_TABLE_PRODUCTS_INFO ." pi on (pi.product_id = p.id and pi.language_code = '". $GLOBALS['system']->language->selected['code'] ."')
