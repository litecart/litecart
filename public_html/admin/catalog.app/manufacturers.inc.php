    ...
    if ($system->database->num_rows($manufacturers_query) > 0) {
      while ($manufacturer = $system->database->fetch($manufacturers_query)) {
        ...
        echo '<tr class="'. $rowclass . ($manufacturer['status'] ? false : ' semi-transparent') .'">' . PHP_EOL
           ...
           . '  <td nowrap="nowrap"><a href="'. $system->document->href_link('', array('app' => $_GET['app'], 'doc' => 'edit_manufacturer', 'manufacturer_id' => $manufacturer['id'])) .'"><img src="'. WS_DIR_IMAGES .'icons/16x16/edit.png" width="16" height="16" align="absbottom" /></a></td>' . PHP_EOL
           . '</tr>' . PHP_EOL;
      }
    }
?>
