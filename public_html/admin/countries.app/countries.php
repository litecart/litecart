...
?>
<div style="float: right;"><?php echo $system->functions->form_draw_link_button($system->document->link('', array('doc' => 'edit_country.php'), true), $system->language->translate('title_add_new_country', 'Add New Country'), '', 'add'); ?></div>
<h1 style="margin-top: 0px;"><img src="<?php echo WS_DIR_ADMIN . $_GET['app'] .'.app/icon.png'; ?>" width="32" height="32" style="vertical-align: middle; margin-right: 10px;" /><?php echo $system->language->translate('title_countries', 'Countries'); ?></h1>
...
<?php
  ...
  // Jump to data for current page
    if ($_GET['page'] > 1) $system->database->seek($countries_query, ($system->settings->get('data_table_rows_per_page') * ($_GET['page']-1)));
...
// Display page links
  echo $system->functions->draw_pagination(ceil($system->database->num_rows($countries_query)/$system->settings->get('data_table_rows_per_page')));
