      ...
      if (!function_exists('custom_draw_category_tree')) {
        function custom_draw_category_tree($categories, $indent=0) {
          ...
          foreach ($categories as $category) {
            echo '  <li class="category-'. $category['id'] . (!empty($category['active']) ? ' active' : '') .'">'. functions::draw_fonticon(!empty($category['opened']) ? 'minus-square' : 'plus-square', 'style="font-size: 0.9em;"') .' <a href="'. htmlspecialchars($category['link']) .'">'. $category['name'] .'</a>';
