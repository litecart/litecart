      ...
      if (!function_exists('custom_draw_category_tree')) {
        function custom_draw_category_tree($categories, $indent=0) {
          ...
          foreach ($categories as $category) {
            echo '  <li class="category-'. $category['id'] . (!empty($category['active']) ? ' active' : '') .'" style="margin-left: '. ($indent*5) .'px;" ><img src="'. WS_DIR_IMAGES .'icons/16x16/'. (!empty($category['opened']) ? 'collapse.png' : 'expand.png') .'" width="16" height="16" alt="" style="vertical-align: middle;" /> <a href="'. htmlspecialchars($category['link']) .'">'. $category['name'] .'</a>';
