    ...
    if (!function_exists('custom_draw_site_menu')) {
      function custom_draw_site_menu($items, $indent=0) {
        ...
        foreach ($items as $item) {
          echo '  <li class="'. $item['type'] .'-'. $item['id'] .'"><a href="'. htmlspecialchars($item['link']) .'">'. (!empty($item['image']) ? '<img src="'. $item['image'] .'" alt="" style="vertical-align: middle; margin-right: 10px;">' : '') .''. $item['title'] .'</a>';
