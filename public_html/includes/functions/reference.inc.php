<?php
  ...
  function reference_in_geo_zone($geo_zone_id, $country_code, $zone_code='') {
    ...
    $zones_to_geo_zones_query = $GLOBALS['system']->database->query(
      ...
      and (country_code = '' or country_code = '". $GLOBALS['system']->database->input($country_code) ."')
      ". ((!empty($zone_code)) ? "and (zone_code = '' or zone_code = '". $GLOBALS['system']->database->input($zone_code) ."')" : "") ."
      limit 1;"
    );
    
    return ($GLOBALS['system']->database->num_rows($zones_to_geo_zones_query) > 0) ? true : false;
  }
  ...
  function reference_get_country_name($code) {
    ...
    $country_query = $GLOBALS['system']->database->query(
      ...
      where iso_code_2 = '". $GLOBALS['system']->database->input($code) ."'
      limit 1;"
    );
  ...
  function reference_get_zone_name($country_code, $code) {
    ...
    $zones_query = $GLOBALS['system']->database->query(
      ...
      and code = '". $GLOBALS['system']->database->input($code) ."'
      limit 1;"
    );
  ...
  function reference_country_num_zones($country_code) {
    ...
    return $GLOBALS['system']->database->num_rows($zones_query);
  }
  ...
  function reference_verify_zone_code($country_code, $code) {
    ...
    return $GLOBALS['system']->database->num_rows($zones_query) ? true : false;
  }
