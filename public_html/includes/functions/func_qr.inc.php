	...
	function qr_generate_svg($data, $size = 200) {
	...
	function qr_place_finder(&$modules, &$is_function, $row, $col, $size) {
			for ($r = -1; $r <= 7; $r++) {
					for ($c = -1; $c <= 7; $c++) {
							$y = $row + $r;
							$x = $col + $c;
	...
	function qr_place_alignment(&$modules, &$is_function, $center_y, $center_x, $size) {
			for ($r = -2; $r <= 2; $r++) {
					for ($c = -2; $c <= 2; $c++) {
							$y = $center_y + $r;
							$x = $center_x + $c;
	...
	function qr_reed_solomon($data, $ec_count) {
			$generators = [
