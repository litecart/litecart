<?php
  ...
  class cart {
    ...
    public static function update($item_key, $quantity) {
      ...
      /*
      if (!empty(self::$data['items'][$item_key]['options']) && ($product->options[self::$data['items'][$item_key]['option_id']]['quantity'] - $quantity) < 0 && empty($product->sold_out_status['orderable'])) {
