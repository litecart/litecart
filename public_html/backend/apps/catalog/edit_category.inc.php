<?php
  ...
  if (isset($_POST['save'])) {
    ...
    try {
      ...
      header('Location: '. document::ilink(null, ['category_id' => $_POST['parent_id']]));
      exit;
  ...
  <div class="card-body">
    <?php echo functions::form_draw_form_begin('category_form', 'post', false, true); ?>
            ...
            <div class="col-md-4">
              <div id="image">
                <div class="thumbnail" style="margin-bottom: 15px;">
                  <img src="<?php echo document::href_link(WS_DIR_STORAGE . functions::image_thumbnail(FS_DIR_STORAGE . 'images/' . $category->data['image'], $category_image_width, $category_image_height, settings::get('category_image_clipping'))); ?>" alt="" />
                </div>
      ...
      <div class="card-action">
        ...
        <?php echo (isset($category->data['id'])) ? functions::form_draw_button('delete', language::translate('title_delete', 'Delete'), 'submit', 'class="btn btn-danger" onclick="if (!window.confirm(\''. language::translate('text_are_you_sure', 'Are you sure?') .'\')) return false;"', 'delete') : false; ?>
      </div>
...
// Image
  ...
  $('input[name="image"]').change(function(e) {
    if ($(this).val() != '') {
      ...
      $('#image img').attr('src', '<?php echo functions::image_thumbnail(FS_DIR_STORAGE . 'images/' . $category->data['image'], $category_image_width, $category_image_height, settings::get('category_image_clipping')); ?>');
    }
  });
