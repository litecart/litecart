...
<div id="modal-category-picker" class="modal fade" style="width: 640px;">
    ...
    <button class="btn btn-primary float-end" name="select" type="button" data-id="<?php echo !empty($_GET['parent_id']) ? (int)$_GET['parent_id'] : '0'; ?>" data-name="<?php echo !empty($_GET['parent_id']) ? reference::category($_GET['parent_id'])->name : language::translate('title_root', 'Root'); ?>" style="margin-inline-start: 1em;">
      <?php echo language::translate('title_select', 'Select'); ?>
    </button>

    <ul class="nav nav-pills" style="margin-bottom: 1em;">
      <li>
        <a href="<?php echo document::ilink(null, ['parent_id' => 0], true); ?>" data-id="0">
          <?php echo language::translate('title_root', 'Root'); ?>
        </a>
      </li>
      <?php foreach ($breadcrumbs as $category) { ?>
      <li>
        <a href="<?php echo document::ilink(null, ['parent_id' => $category['id'], true]); ?>" data-id="<?php echo $category['id']; ?>">
      ...
      <li>
        <a href="<?php echo document::ilink(null, ['parent_id' => reference::category($_GET['parent_id'])->parent_id], true); ?>">
      ...
      <li>
        <a href="<?php echo document::ilink(null, ['parent_id' => $category['id']], true); ?>">
