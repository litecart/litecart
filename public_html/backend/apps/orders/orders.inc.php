				...
				<tr class="<?php echo implode(' ', $order['css_classes']); ?>" data-id="<?php echo $order['id']; ?>">
					...
					<td>
						<a class="link" href="<?php echo document::href_ilink(__APP__.'/edit_order', ['order_id' => $order['id'], 'redirect_url' => $_SERVER['REQUEST_URI']]); ?>">
							<?php echo functions::draw_fonticon($order['billing_company'] ? 'icon-building' : 'icon-user', 'style="opacity: .5;"'); ?>
							<?php echo $order['billing_company'] ?: $order['billing_firstname'] .' '. $order['billing_lastname']; ?><?php if (!$order['customer_id']) echo ' <em>('. language::translate('title_guest', 'Guest') .')</em>'; ?>
						</a>
					</td>
					<td><?php echo $order['billing_country_code']; ?></td>
