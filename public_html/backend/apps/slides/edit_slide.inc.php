...
<div class="card card-app">
  ...
  <div class="card-body">
    <?php echo functions::form_draw_form_begin('slide_form', 'post', false, true, 'style="max-width: 640px;"'); ?>
      ...
      <?php if (!empty($slide->data['image'])) echo '<p><img src="'. document::href_link(WS_DIR_STORAGE . 'images/' . $slide->data['image']) .'" alt="" class="img-responsive" /></p>'; ?>

      <div class="form-group">
        <label><?php echo language::translate('title_image', 'Image'); ?></label>
        <?php echo functions::form_draw_file_field('image', 'accept="image/*,.svg"'); ?>
      ...
      <div class="card-action">
        ...
        <?php echo (!empty($slide->data['id'])) ? functions::form_draw_button('delete', language::translate('title_delete', 'Delete'), 'submit', 'formnovalidate class="btn btn-danger" onclick="if (!window.confirm(\''. language::translate('text_are_you_sure', 'Are you sure?') .'\')) return false;"', 'delete') : false; ?>
