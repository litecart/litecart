<?php
  $box_brand_links_cache_token = cache::token('box_brand_links', ['language', isset($_GET['brand_id']) ? $_GET['brand_id'] : '']);
  if (cache::capture($box_brand_links_cache_token)) {
    ...
    if (database::num_rows($brands_query)) {
      ...
      $box_brand_links = new ent_view('partials/box_brand_links.inc.php');
