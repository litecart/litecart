<?php
  ...
  if (cache::capture($box_manufacturer_logotypes_cache_token)) {
    ...
    $manufacturers_query = database::query(
      "select id, image, name from ". DB_PREFIX ."manufacturers
      where status
      and featured
      and image != ''
      order by rand();"
      ...
      while ($manufacturer = database::fetch($manufacturers_query)) {
        $box_manufacturer_logotypes->snippets['logotypes'][] = [
          ...
          'image' => [
            ...
            'thumbnail' => functions::image_thumbnail(FS_DIR_STORAGE . 'images/' . $manufacturer['image'], 0, 30, 'FIT'),
            'thumbnail_2x' => functions::image_thumbnail(FS_DIR_STORAGE . 'images/' . $manufacturer['image'], 0, 60, 'FIT'),
          ],
        ];
      }
