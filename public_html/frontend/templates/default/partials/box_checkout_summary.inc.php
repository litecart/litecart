...
<section id="box-checkout-summary" class="box">
  ...
  <table class="table table-striped table-bordered data-table">
    <tbody>
      ...
      <tr>
        <td colspan="3" class="text-end text-muted"><?php echo language::translate('title_discount', 'Discount'); ?>:</td>
        <td class="text-end text-muted"><?php echo currency::format($order['discount'], false, $order['currency_code'], $order['currency_value']); ?></td>
      </tr>
      <?php } ?>
...
// Summary Form: Process Data
  ...
  $('form[name="checkout_form"]').submit(function(e) {
    ...
    $('#box-checkout-summary button[name="confirm_order"]').css('display', 'none').before(new_button);
  });
</script>