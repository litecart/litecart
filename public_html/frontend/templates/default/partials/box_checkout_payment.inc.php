<section id="box-checkout-payment" class="box box-default">
  ...
  <div class="options">
    ...
    <label class="option text-start<?php echo (!empty($selected['id']) && $selected['id'] == $option['id']) ? ' active' : ''; ?><?php echo !empty($option['error']) ? ' disabled' : ''; ?>">
      ...
      <input name="payment_option[id]" value="<?php echo htmlspecialchars($option['id']); ?>" type="radio" hidden<?php if (!empty($option['error'])) echo ' disabled'; ?><?php if (!empty($selected['id'])) echo ' checked'; ?> />
