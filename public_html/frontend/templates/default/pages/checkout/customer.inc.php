...
<main class="container">
	...
	<section id="box-checkout-customer" class="card" style="max-width: 720px; margin: 0 auto;">
		...
		<div class="card-body">
			...
			<?php echo f::form_begin('customer_form', 'post'); ?>
					...
					<div class="grid">
						...
						<div class="col-sm-6">
							<label class="form-group">
								...
								<?php echo f::form_input_text('customer[tax_id]', true, (isset($_POST['customer']['type']) && $_POST['customer']['type'] == 'individual') ? 'disabled' : ''); ?>
							</label>
						</div>
						<?php } ?>
					</div>
				...
				<div class="grid">
					...
					<div class="col-sm-6">
						<label class="form-group">
							...
							<?php echo f::form_select_zone('customer[zone_code]', $_POST['country_code'] ?? customer::$data['country_code'] ?? settings::get('store_country_code'), true); ?>
						</label>
					</div>
					<?php } ?>
				</div>
					...
					<fieldset<?php echo (empty($_POST['different_shipping_address'])) ? ' style="display: none;" disabled' : ''; ?>>
						...
						<div class="grid">
							...
							<div class="col-sm-6">
								<label class="form-group">
									<div class="form-label"><?php echo t('title_zone_state_province', 'Zone/State/Province'); ?></div>
									<?php echo f::form_select_zone('shipping_address[zone_code]', $_POST['shipping_address']['country_code'] ?? customer::$data['shipping_address']['country_code'] ?? $_POST['country_code'] ?? customer::$data['country_code'] ?? settings::get('store_country_code'), true); ?>
								</label>
							</div>
							<?php } ?>
						</div>
				...
				<div class="account">
					...
					<div class="notices">
						<div class="notice notice-default">
							<?php echo f::draw_fonticon('icon-info'); ?> <?php echo t('notice_sign_in_to_connect_purchase_to_account', 'Sign in to connect this purchase to your account'); ?>
						</div>
					</div>
