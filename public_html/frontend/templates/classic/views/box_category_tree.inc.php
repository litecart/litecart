<?php
  $draw_branch = function($category, &$category_path) use (&$draw_branch) {
    ...
    echo '<li class="category-'. $category['id'] . (!empty($category['opened']) ? ' opened' : '') . (!empty($category['active']) ? ' active' : '') .'">' . PHP_EOL
       . '  <a href="'. htmlspecialchars($category['link']) .'">'. functions::draw_fonticon(!empty($category['opened']) ? 'fa-angle-down fa-fw pull-right' : 'fa-angle-right fa-fw pull-right') .' '. $category['name'] . ((settings::get('category_tree_product_count') && $category['num_products']) ? ' <small class="pull-right">('. $category['num_products'] .')</small>' : '') .'</a>' . PHP_EOL;
