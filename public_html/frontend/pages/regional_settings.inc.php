    ...
    try {
      ...
      if (!empty($_POST['country_code'])) {
        ...
        if (!empty($_COOKIE['cookies_accepted'])) {
          header('Set-Cookie: country_code='. customer::$data['country_code'] .'; Path='. WS_DIR_APP .'; Expires='. gmdate('r', strtotime('+3 months')) .'; SameSite=Strict');
          header('Set-Cookie: zone_code='. customer::$data['zone_code'] .'; Path='. WS_DIR_APP .'; Expires='. gmdate('r', strtotime('+3 months')) .'; SameSite=Strict');
        }
      }
      ...
      if (isset($_POST['display_prices_including_tax'])) {
        ...
        if (!empty($_COOKIE['cookies_accepted'])) {
          header('Set-Cookie: display_prices_including_tax='. (int)customer::$data['display_prices_including_tax'] .'; Path='. WS_DIR_APP .'; Expires='. gmdate('r', strtotime('+3 months')) .'; SameSite=Strict');
        }
      }
