<?php
	...
	document::$title[] = t('title_sign_up', 'Sign Up');

	breadcrumbs::add(t('title_sign_up', 'Sign Up'), document::ilink('account/sign_up'));
	...
	if (customer::check_login()) {
		notices::add('errors', t('error_already_logged_in', 'You are already logged in'));
	}
	...
	if (!empty($_POST['sign_up'])) {
		...
		try {
			...
			if (database::query(
				...
				throw new Exception(t('error_email_already_registered', 'The email address already exists in our customer database. Please login or select a different email address.'));
			}
