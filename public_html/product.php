    ...
    foreach ($product->images as $image) {
      if ($first_image) {
        ...
        echo '<div style="position: relative;">' . PHP_EOL
           . '  <a href="'. WS_DIR_IMAGES . $image .'" class="fancybox" data-fancybox-group="product"><img src="'. functions::image_resample(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . $image, FS_DIR_HTTP_ROOT . WS_DIR_CACHE, 640, 640, 'FIT_USE_WHITESPACING') .'" class="main-image zoomable shadow rounded-corners" alt="" title="'. htmlspecialchars($product->name[language::$selected['code']]) .'" itemprop="image" /></a>' . PHP_EOL
           . '  '. $sticker . PHP_EOL
           . '</div>' . PHP_EOL;
        $first_image = false;
      } else {
        echo '<div style="display: inline;"><a href="'. WS_DIR_IMAGES . $image .'" class="fancybox" data-fancybox-group="product"><img src="'. functions::image_resample(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . $image, FS_DIR_HTTP_ROOT . WS_DIR_CACHE, 180, 180, 'CROP') .'" class="extra-image zoomable shadow" title="'. htmlspecialchars($product->name[language::$selected['code']]) .'" /></a></div>';
      }
    }
  } else {
    echo '<img src="'. functions::image_resample(FS_DIR_HTTP_ROOT . WS_DIR_IMAGES . 'no_image.png', FS_DIR_HTTP_ROOT . WS_DIR_CACHE, 320, 320, 'FIT_USE_WHITESPACING') .'" class="extra-image" alt="" />' . PHP_EOL;
  }
?>
    </div>
...
?>
  ...
  ?>
      ...
      <?php if (settings::get('display_cheapest_shipping')) { ?>
...
?>
      ...
      <div style="margin-bottom: 20px;" class="social-bookmarks">
