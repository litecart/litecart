...
ORDER BY id ASC;
-- --------------------------------------------------------
INSERT IGNORE INTO `lc_stock_items`
(product_id, sku, mpn, gtin, taric, quantity)
SELECT id, sku, mpn, gtin, taric, quantity
FROM `lc_products`
WHERE id NOT IN (
  SELECT DISTINCT product_id FROM `lc_stock_items`
);
-- --------------------------------------------------------
