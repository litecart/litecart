...
// Delete files
  ...
  $modified_files = [
    [
      ...
      'search'  => "  define('DB_TABLE_PRODUCTS_OPTIONS',                  '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'products_options`');" . PHP_EOL,
      'replace' => "  define('DB_TABLE_PRODUCTS_OPTIONS',                  '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'products_options`');" . PHP_EOL
                 . "  define('DB_TABLE_PRODUCTS_OPTIONS_VALUES',           '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'products_options_values`');" . PHP_EOL,
    ],
    [
      'file'    => FS_DIR_APP . 'includes/config.inc.php',
      'search'  => "  define('DB_TABLE_OPTION_GROUPS',                     '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'option_groups`');" . PHP_EOL
                 . "  define('DB_TABLE_OPTION_GROUPS_INFO',                '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'option_groups_info`');" . PHP_EOL
                 . "  define('DB_TABLE_OPTION_VALUES',                     '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'option_values`');" . PHP_EOL
                 . "  define('DB_TABLE_OPTION_VALUES_INFO',                '`'. DB_DATABASE .'`.`'. DB_PREFIX . 'option_values_info`');" . PHP_EOL,
      'replace' => "" . PHP_EOL,
    ],
  ];
...
// Move option groups into attribute groups
  ...
  if ($option_group = database::fetch($option_groups_query)) {
    ...
    // Make certain the attribute group id does not collide with a previous option group id
      while (database::num_rows(database::query("select id from ". DB_PREFIX ."option_groups where id = ". (int)$attribute_group_id ." limit 1;"))) {
        database::query(
      ...
      database::query(
        "insert into ". DB_PREFIX ."attribute_groups_info (group_id, language_code, name)
  ...
  // Update values in products_options and products_options_values
    ...
    while ($option_value = database::fetch($option_values_query)) {
      ...
      // Make certain the attribute value id does not collide with a previous option value id
        while (database::num_rows(database::query("select id from ". DB_PREFIX ."option_values where id = ". (int)$attribute_value_id ." limit 1;"))) {
          database::query(
        ...
        database::query(
          "insert into ". DB_PREFIX ."attribute_values_info (value_id, language_code, name)
