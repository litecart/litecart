<?php
  ...
  try {
    ...
    define('FS_DIR_APP', realpath(__DIR__ .'/../') .'/');
    define('FS_DIR_STORAGE', FS_DIR_APP);

    if (!empty($_SERVER['DOCUMENT_ROOT'])) {
      define('WS_DIR_APP', preg_replace('#^'. preg_quote(rtrim(realpath($_SERVER['DOCUMENT_ROOT']), '/'), '#') .'#', '', FS_DIR_APP));
    } else if (php_sapi_name() == 'cli' && !empty($_REQUEST['document_root'])) {
      define('WS_DIR_APP', preg_replace('#^'. preg_quote(rtrim(realpath($_REQUEST['document_root']), '/'), '#') .'#', '', FS_DIR_APP));
    } else {
