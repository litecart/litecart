...
('1', 'si', 'sin', 'සිංහල', 'si_LK.UTF-8', 'UTF-8', '%Y %B %e', '%Y %b %e', '%H:%M', '%Y %b %e %H:%M', '.', ',', 'LKR', '2', '', 'ltr', NOW(), NOW());
-- -----
INSERT INTO `lc_currencies` (`status`, `code`, `number`, `name`, `value`, `decimals`, `prefix`, `suffix`, `priority`, `date_updated`) VALUES
('1', 'LKR', '144', 'Sri Lankan Rupee', '1.00000000', '2', '₨', '', '1', NOW());
-- -----
ALTER TABLE `lc_products` ADD `price_lkr` DECIMAL(11,4) NOT NULL AFTER `price_usd`;
-- -----
ALTER TABLE `lc_products` ADD `campaign_price_lkr` DECIMAL(11,4) NOT NULL AFTER `campaign_price_usd`;
-- -----
...
VALUES(@TAX_CLASS_ID, @VAT_ZONE_ID, 'percent', 'Sri Lanka VAT', '15%', '15.00000', NOW(), NOW());