...
('1', 'ne', 'nep', 'नेपाली', 'ne_NP.UTF-8', 'UTF-8', '%e %B %Y', '%e %b %Y', '%H:%M', '%e %b %Y %H:%M', '.', ',', 'NPR', '2', '', 'ltr', NOW(), NOW());
-- -----
INSERT INTO `lc_currencies` (`status`, `code`, `number`, `name`, `value`, `decimals`, `prefix`, `suffix`, `priority`, `date_updated`) VALUES
('1', 'NPR', '524', 'Nepalese Rupee', '1.00000000', '2', '₨', '', '1', NOW());
-- -----
ALTER TABLE `lc_products` ADD `price_npr` DECIMAL(11,4) NOT NULL AFTER `price_usd`;
-- -----
ALTER TABLE `lc_products` ADD `campaign_price_npr` DECIMAL(11,4) NOT NULL AFTER `campaign_price_usd`;
-- -----
...
VALUES(@TAX_CLASS_ID, @VAT_ZONE_ID, 'percent', 'Nepal VAT', '13%', '13.00000', NOW(), NOW());