(function($,window,i){$.fn.responsiveSlides=function(options){var settings=$.extend({auto:true,speed:500,timeout:4000,pager:false,nav:false,random:false,pause:false,pauseControls:true,prevText:'Previous',nextText:'Next',maxwidth:'',navContainer:'',manualControls:'',namespace:'rslides',before:$.noop,after:$.noop},options);return this.each(function(){i++;var $this=$(this),vendor,selectTab,startCycle,restartCycle,rotate,$tabs,index=0,$slide=$this.children(),length=$slide.length,fadeTime=parseFloat(settings.speed),waitTime=parseFloat(settings.timeout),maxw=parseFloat(settings.maxwidth),namespace=settings.namespace,namespaceIdx=namespace+i,navClass='nav '+namespaceIdx+'_nav',activeClass=namespace+'_here',visibleClass=namespaceIdx+'_on',slideClassPrefix=namespaceIdx+'_s',$pager=$('<ul class="'+namespace+'_tabs '+namespaceIdx+'_tabs" />'),visible={float:'left',position:'relative',opacity:1},hidden={float:'none',position:'absolute',opacity:0},supportsTransitions=(function(){var docBody=document.body||document.documentElement;var styles=docBody.style;var prop="transition";if(typeof styles[prop]==="string"){return true;}
