<?php
  ...
  $manufacturers_cache_token = cache::token('manufacturers', ['basename', 'get', 'language', 'currency', 'account', 'prices']);
  if (cache::capture($manufacturers_cache_token)) {
    ...
    $manufacturers_query = database::query(
      ...
      left join ". DB_PREFIX ."manufacturers_info mi on (mi.manufacturer_id = m.id and mi.language_code = '". language::$selected['code'] ."')
      where status
      order by name;"
    );
