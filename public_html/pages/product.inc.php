<?php
  ...
  $snippets = array(
    ...
    'excluding_tax' => language::translate('title_excluding_tax', 'Excluding Tax'),
    'tax_rates' => array(),
    'quantity' => $product->quantity,
    'quantity_title' => language::translate('title_quantity', 'Quantity'),
    'stock_status_title' => language::translate('title_stock_status', 'Stock Status'),
    ...
    'delivery_status_title' => language::translate('title_delivery_status', 'Delivery Status'),
...
// Tax
  foreach (tax::get_tax_by_rate($product->campaign['price'] ? $product->campaign['price'] : $product->price, $product->tax_class_id) as $tax_rate) {
    $snippets['tax_rates'][] = currency::format($tax_rate['tax']) .' ('. $tax_rate['name'] .')';
  }
...
// Cheapest shipping
  if (settings::get('display_cheapest_shipping')) {
    ...
    if (!empty($cheapest_shipping)) {
      ...
      $snippets['cheapest_shipping'] = str_replace('%price', currency::format(tax::calculate($shipping_cost, $shipping_tax_class_id)), language::translate('text_cheapest_shipping_from_price', 'Cheapest shipping from %price'));
    }
  }
...
// Options
  ...
  ob_start();
  include vqmod::modcheck(FS_DIR_HTTP_ROOT . WS_DIR_BOXES . 'similar_products.inc.php');
  $snippets['box_similar_products'] = ob_get_clean();

  ob_start();  
  include vqmod::modcheck(FS_DIR_HTTP_ROOT . WS_DIR_BOXES . 'also_purchased_products.inc.php');
  $snippets['box_also_purchased_products'] = ob_get_clean();
  
  echo document::stitch('file', 'product', $snippets);
  //exit;
?>