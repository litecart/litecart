<?php

	include_once __DIR__.'/../src/shared/app_header.inc.php';

	try {

		// Start a MySQL transaction so we can rollback the test
		database::begin_transaction();

		// Fetch the current auto increment ID
		$auto_increment_id = database::query(
			"SHOW TABLE STATUS LIKE '". DB_PREFIX ."quantity_units';"
		)->fetch('Auto_increment');

		// Prepare some example data
		$data = [
			'name' => ['en' => 'Fictional Unit'],
			'description' => ['en' => 'Unit of fictional mass'],
			'decimals' => 3,
		];

		########################################################################
		## Creating a new quantity unit
		########################################################################

		// Create a new entity
		$quantity_unit = new ent_quantity_unit();
		$quantity_unit->data = f::array_update($quantity_unit->data, $data);
		$quantity_unit->save();

		// Check if the entity was created
		if (!$quantity_unit_id = $quantity_unit->data['id']) {
			throw new Exception('Failed to create quantity unit');
		}

		########################################################################
		## Load and check the quantity unit
		########################################################################

		// Load the entity
		$quantity_unit = new ent_quantity_unit($quantity_unit_id);

		// Check if the quantity unit was loaded
		if ($quantity_unit->data['id'] != $quantity_unit_id) {
			throw new Exception('Failed to load quantity unit');
		}

		// Check if data was set correctly
		if (!f::array_intersect_compare($data, $quantity_unit->data)) {
			throw new Exception('The quantity unit data was not stored correctly');
		}

		########################################################################
		## Updating the quantity unit
		########################################################################

		// Prepare some new data
		$data = [
			'name' => ['en' => 'Gram'],
			'description' => ['en' => 'Unit of mass'],
			'decimals' => 2,
		];

		// Update some data
		$quantity_unit->data = f::array_update($quantity_unit->data, $data);

		// Save changes to database
		$quantity_unit->save();

		// Check if data was set correctly
		if (!f::array_intersect_compare($data, $quantity_unit->data)) {
			throw new Exception('The quantity unit data was not updated correctly ');
		}

		########################################################################
		## Deleting the quantity unit
		########################################################################

		// Delete the entity
		$quantity_unit->delete();

		// Check if the entity was deleted
		if (database::query(
			"select id from ". DB_PREFIX ."quantity_units
			where id = ". (int)$quantity_unit_id ."
			limit 1;"
		)->num_rows) {
			throw new Exception('Failed to delete quantity unit');
		}

		return true;

	} catch (Exception $e) {

		echo ' [Failed]'. PHP_EOL . 'Error: '. $e->getMessage();
		return false;

	} finally {

		// Rollback changes to the database
		database::rollback();

		// Revert the auto increment ID
		database::query(
			"ALTER TABLE ". DB_PREFIX ."quantity_units
			AUTO_INCREMENT = ". (int)$auto_increment_id .";"
		);
	}
