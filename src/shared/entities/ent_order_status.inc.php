<?php
	...
	class ent_order_status {
		...
		public function reset(): void {
			...
			foreach (database::schema(DB_PREFIX .'order_statuses') as $field) {
		...
		public function save(): void {
			...
			if (!$this->data['id']) {
				...
				database::insert('order_statuses', [
			...
			database::query(
				...
				set hidden = '". (empty($this->data['hidden']) ? '0' : '1') ."',
					...
					email_subject = '". database::input(f::format_json($this->data['email_subject'])) ."',
					email_message = '". database::input(f::format_json($this->data['email_message'])) ."',
