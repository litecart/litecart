<?php
	...
	function captcha_draw(string $id='default', array $config=[], string|array $attributes=''): string|bool {
		...
		return implode(PHP_EOL, [
			...
			'  ' . form_input_text('lc-captcha-response', '', ['required' => true, 'maxlength' => (int)$config['length'], 'autocomplete' => 'off', 'style' => 'font-size: '. round($config['font_size']) .'px; padding: 0; text-align: center;', ...$attributes]),
			'</div>',
		]);
	}
