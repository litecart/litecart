<?php
	...
	if (database::query(
		"SHOW COLUMNS FROM ". DB_PREFIX ."customers
		LIKE 'num_logins';"
	)->num_rows) {
		database::query(
			"ALTER TABLE ". DB_PREFIX ."customers
			CHANGE COLUMN `num_logins` `total_logins` INT(11) NOT NULL DEFAULT '0' AFTER `login_attempts`;"
		);
	}

	if (!database::query(
		"SHOW COLUMNS FROM ". DB_PREFIX ."newsletter_recipients
		LIKE 'client_ip';"
	)->num_rows) {
		database::query(
			"ALTER TABLE ". DB_PREFIX ."newsletter_recipients
	...
	database::query(
		...
		FROM ". DB_PREFIX ."products_options_stock
		GROUP BY product_id;"
