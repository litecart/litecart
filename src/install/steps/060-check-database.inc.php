<?php
	...
	try {
		...
		if (!extension_loaded('mysqli')) {
			throw new Exception('MySQLi is not installed or configured for PHP');
		}

		if (!database::connect('default', DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_DATABASE, DB_COLLATION)) {
	...
	try {
		...
		if (version_compare($database_software['version'], $database_software['min_version'], '<')) {
			throw new Exception($database_software['name'] .' '. $database_software['min_version'] .'+ required');
		}

		if (version_compare($database_software['version'], $database_software['recommended_version'], '<')) {
	...
	try {
		...
		if (!$charset) {
			throw new Exception('Failed to retrieve character set');
		}
