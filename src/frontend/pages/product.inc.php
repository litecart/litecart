		...
		switch ($group['function']) {
			...
			case 'checkbox':
				...
				foreach ($group['values'] as $value) {
					...
					$values .= f::form_checkbox('customizations['.$group['name'].'][]', [$value['name'], $value['name'] . $price_adjustment_text], true, ['data-group-id' => (int)$group['group_id'], 'data-value-id' => (int)$value['value_id'], 'data-price-adjust' => (float)$price_adjustment, 'data-tax-adjust' => (float)$tax_adjust] + (!empty($group['required']) ? ['required' => true] : []));
				}
			...
			case 'radio':
				...
				foreach ($group['values'] as $value) {
					...
					$values .= implode(PHP_EOL, [
						'<div class="radio">',
						'  <label>'. f::form_radio_button('customizations['.$group['name'].']', $value['name'], true, ['data-group-id' => (int)$group['group_id'], 'data-value-id' => (int)$value['value_id'], 'data-price-adjust' => (float)$price_adjustment, 'data-tax-adjust' => (float)$tax_adjust] + (!empty($group['required']) ? ['required' => true] : [])) .' '. $value['name'] . $price_adjustment_text . '</label>',
						'</div>',
					]);
				}
			...
			case 'select':
				...
				$values .= f::form_select('customizations['.$group['name'].']', $customizations, true, ['data-group-id' => (int)$group['group_id']] + (!empty($group['required']) ? ['required' => true] : []));
				break;

			case 'text':

				$values .= f::form_input_text('customizations['.$group['name'].']', true, ['data-group-id' => (int)$group['group_id']] + (!empty($group['required']) ? ['required' => true] : [])) . PHP_EOL;
				break;
